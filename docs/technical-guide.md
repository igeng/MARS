...
│  │  领域分析师   │  │  论文检索师  │  │  深度分析师  │           │
│  │  LLM: Qwen   │  │ LLM: DeepSeek│  │  LLM: Kimi   │           │
│  └──────────────┘  └──────────────┘  └──────────────┘           │
│                                                                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐           │
│  │  Connector    │  │  Summarizer  │  │  Evaluator   │           │
│  │  关联分析师   │  │  综述生成师  │  │  质量评估师  │           │
│  │  LLM: GLM    │  │  LLM: Qwen   │  │ LLM: DeepSeek│           │
│  └──────────────┘  └──────────────┘  └──────────────┘           │
└──────────────────────────┬───────────────────────────────────────┘
                           │
                           ▼
...
│   ├── services/                  # 服务层
│   │   ├── __init__.py
│   │   └── llm_gateway.py         # LLM 网关（4 家供应商路由）
│   │
│   ├── agents/                    # 6 个智能体
│   │   ├── __init__.py
│   │   ├── researcher.py          # 领域分析师
...
cli.py / api/main.py
    └── crews/ (search_crew, analysis_crew, connection_crew, full_research_crew)
          ...
          │     └── services/llm_gateway.py (get_llm_by_task)
          │     └── tools/ (各种工具)
...
| LLM | DeepSeek-V3（均衡能力） |
| 工具 | `DBLPSearchTool`, `SemanticScholarSearchTool`, `ArXivSearchTool`, `KeywordExpanderTool` |
| max_iter | 8 |
| 输入 | 领域分析结果 + 关键词 |
| 输出 | 结构化论文列表（20-50 篇） |
...
`mars/tasks/task_definitions.py` 提供 6 个任务工厂函数：
...
def create_review_generation_task(agent, topic, *, context=None) -> Task
...
create_review_generation_task    ← context = [全部前置任务]
```
...
```
Researcher ──(domain_analysis_task)──→ Searcher ──(paper_search_task)──→ 结果
```
...
```
Connector ──(connection_analysis_task)──→ Summarizer ──(review_generation_task)──→ 结果
```
...
```
Phase 1:  Researcher → Searcher
Phase 2:  Analyzer → Connector → Evaluator (带上下文依赖)
Phase 3:  Summarizer (综合所有结果)
```

- Agent 数量：6
- Task 数量：6 + 1 综合综述
- 执行模式：Sequential（CrewAI 通过 context 实现逻辑并行）
- 输出：完整研究报告（≥2000 字）
...
| 供应商 | 函数 | API 端点 | 模型 |
|--------|------|----------|------|
| 阿里云（Qwen） | `get_qwen_llm()` | `https://dashscope.aliyuncs.com/compatible-mode/v1` | qwen-max |
| DeepSeek | `get_deepseek_llm()` | `https://api.deepseek.com/v1` | deepseek-chat |
| 月之暗面（Kimi） | `get_kimi_llm()` | `https://api.moonshot.cn/v1` | moonshot-v1-8k |
| 智谱 AI（GLM） | `get_glm_llm()` | `https://open.bigmodel.cn/api/paas/v4` | glm-4-plus |

所有供应商均通过 OpenAI 兼容 API 接口访问，统一返回 `ChatOpenAI` 实例。

### 8.3 智能体 → LLM 映射

```python
_AGENT_LLM_MAP = {
    "researcher":  ("qwen", None),      # 强推理 → Qwen-Max
    "searcher":    ("deepseek", None),   # 均衡 → DeepSeek-V3
    "analyzer":    ("kimi", None),       # 长上下文 → Kimi
    "connector":   ("glm", None),        # 关系推理 → GLM-4-Plus
    "summarizer":  ("qwen", None),       # 长文本生成 → Qwen-Max
    "evaluator":   ("deepseek", None),   # 评估 → DeepSeek-V3
}
```
...
# 通过供应商名获取
llm = get_llm(provider="qwen", temperature=0.3)

# 通过智能体角色获取（推荐）
llm = get_llm_by_task("researcher", temperature=0.3)
```
...
| 命令 | 说明 | 示例 |
|------|------|------|
| `mars search <topic>` | 基础检索 | `mars search "federated learning"` |
...
│  └─ 输出: {research_domain, sub_domains, keywords,       │
│            recommended_venues}                            │
│                         │                                 │
│                         ▼                                 │
│  Searcher Agent                                          │
...
│  └─ 输出: [{title, authors, venue, year, citations,      │
│            url, relevance_reason}, ...] (50 papers)       │
│                                                           │
└──── Phase 2 ─────────────────────────────────────────────┐
│                                                           │
...
│  Summarizer Agent                                        │
│  ├─ 综合所有 Phase 1/2 结果                              │
│  ├─ 输出: comprehensive_review.md (≥2000 字)            │
│                                                           │
└──────────────────────────────────────────────────────────┘
```
...
| 文件 | 测试内容 | 测试数量 |
|------|----------|----------|
| `tests/test_basic.py` | 配置、工具、Agent/Crew 导入 | ~46 |
| `tests/test_new_components.py` | Settings、LLM Gateway、DB、API、Tasks | ~33 |
...
| LLM 接口 | LangChain OpenAI | ≥0.2.0 | 统一 LLM 调用接口 |
